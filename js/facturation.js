/* js/facturation.js - Logique Complète (Avec votre PDF vert) */
...
onAuthStateChanged(auth, (user) => {
    if (!user) {
        ...
        // Initialiser la date du jour pour les dépenses
        const dateInput = document.getElementById('dep_date_fac');
...
// ==========================================================================
// 2. VOTRE MOTEUR PDF (INTÉGRÉ ICI)
// ==========================================================================
window.generatePDFFromData = function(data, saveMode = false) {
    ...
    // Récupération sécurisée des paiements
    const paymentsList = data.paiements || [];
    const dejaRegle = paymentsList.reduce((sum, p) => sum + (parseFloat(p.montant) || 0), 0);
    ...
    let yReste = curY + 12; // Position de base
    
    // Affichage des lignes de paiement (Espèces/Virement)
    if(paymentsList.length > 0) {
        ...
        yReste = yPay + 5; // On descend le cadre vert sous les paiements
    }

    // Reste à Payer (Cadre Vert)
    ...
    if (typeDoc === 'FACTURE') {
        ...
        doc.text("Coordonnées Bancaires :", 18, curY + 14);
        
        doc.setFont("helvetica", "normal");
        doc.text("Banque : BANQUE POPULAIRE DU SUD", 18, curY + 19);
        doc.text("Titulaire : EI POMPES FUNEBRES SOLIDAIRE", 18, curY + 24);
        ...
        // MODE DEVIS (Signature)
         ...
         doc.text("Le ........................................", 20, ySig + 16); 
         doc.text("à ........................................", 80, ySig + 16);
    ...
    // 5. MENTIONS LÉGALES
    let footerY = Math.max(curY, yReste + 15);
    if (typeDoc === 'FACTURE') footerY = Math.max(curY, yReste + 15);

    doc.setFontSize(8); doc.setFont("helvetica", "bold"); doc.setTextColor(220, 38, 38); // Rouge
    doc.text("NB :", 15, footerY); doc.setTextColor(0); doc.setFont("helvetica", "normal");
    doc.text("- (NA) TVA non applicable, art 293 B du CGI", 25, footerY);
    doc.text("- (*) Prestations et fournitures obligatoires", 25, footerY + 4);
    
    if (typeDoc === 'DEVIS') {
         footerY += 9;
         doc.setFont("helvetica", "bold"); doc.setFontSize(7);
         const mentionDevis = "Conformément à l'article 5 de l'arrêté du 11 janvier 1999, lorsque le devis est accepté, un bon de commande est alors établi et signé par le client.";
         doc.text(doc.splitTextToSize(mentionDevis, 180), 15, footerY);
    }
...
window.switchTab = function(tab) {
    ...
    if(tab === 'factures') { 
        ...
        document.getElementById('btn-tab-achats').classList.add('active-red'); 
        window.chargerDepenses(); 
    }
};
...
window.nouveauDocument = function() {
    ...
    // Cacher bouton facturer
    const btnTransform = document.getElementById('btn-transform');
    if(btnTransform) btnTransform.classList.add('hidden');
...
window.chargerListeFactures = async function() {
    ...
    try {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            if (typeDoc === "FACTURE" && dateDoc.getFullYear() === currentYear) { 
                global_CA += totalDoc; 
            } 
        ...
        window.chargerDepenses(); // Charger les dépenses après pour les stats
    } catch(e) { 
        console.error(e); 
        tbody.innerHTML = `<tr><td colspan="8" style="color:red;">Erreur : ${e.message}</td></tr>`; 
    }
};
...
window.chargerDocument = async (id) => { 
    ...
    if(d.exists()) {
        ...
        // Afficher bouton "Facturer" si c'est un devis
        const btnTransform = document.getElementById('btn-transform');
        if(btnTransform) {
            if(data.info.type === 'DEVIS') btnTransform.classList.remove('hidden');
            else btnTransform.classList.add('hidden');
        }
...
window.ajouterSection = function(titre="NOUVELLE SECTION") { 
    const tr = document.createElement('tr'); tr.className = "row-section"; 
    tr.innerHTML = `<td class="drag-handle"><i class="fas fa-grip-vertical"></i></td><td colspan="4"><input type="text" class="input-cell input-section" value="${titre}"></td><td style="text-align:center;"><i class="fas fa-trash" style="color:#ef4444; cursor:pointer;" onclick="this.closest('tr').remove(); window.calculTotal();"></i></td>`; 
    document.getElementById('tbody_lignes').appendChild(tr); 
};
...
window.sauvegarderDocument = async function() {
    ...
    try {
        if(id) {
            ...
            // Génération de numéro auto si création
            const q = query(collection(db, "factures_v2")); 
            const snap = await getDocs(q);
            const count = snap.size + 1; 
            const year = new Date().getFullYear(); 
            const prefix = (docData.info.type === 'DEVIS') ? 'D' : 'F';
...
window.supprimerDocument = async (id) => { 
...
window.visualiserPDF = async (id) => { 
    const d = await getDoc(doc(db,"factures_v2",id)); 
    if(d.exists()) window.generatePDFFromData(d.data()); 
};
...
window.ajouterPaiement = () => { 
    const p = { 
        ...
        montant: parseFloat(document.getElementById('pay_amount').value) 
    }; 
    if(p.montant > 0) { 
        paiements.push(p); 
        window.renderPaiements(); 
...
window.supprimerPaiement = (index) => { 
...
window.renderPaiements = () => { 
    ...
    paiements.forEach((p, i) => { 
        div.innerHTML += `<div>${p.date} - ${p.mode}: <strong>${p.montant}€</strong> <i class="fas fa-trash" style="color:red;cursor:pointer;margin-left:10px;" onclick="window.supprimerPaiement(${i})"></i></div>`; 
    }); 
};
...
window.filtrerDepenses = function() {
    ...
    cacheDepenses.filter(d => (d.fournisseur+d.details+d.categorie).toLowerCase().includes(term)).forEach(data => {
        const tr = document.createElement('tr');
        const badgeClass = data.statut === 'Réglé' ? 'badge-regle' : 'badge-attente';
        tr.innerHTML = `
            <td>${new Date(data.date).toLocaleDateString()}</td>
            <td><strong>${data.fournisseur}</strong><br><small>${data.categorie}</small></td>
            <td>${data.reference || '-'}</td>
            <td><span class="badge ${badgeClass}">${data.statut}</span></td>
            ...
            <td style="text-align:center;">
                ...
                <button class="btn-icon" onclick="window.supprimerDepense('${data.id}')"><i class="fas fa-trash" style="color:red;"></i></button>
            </td>`;
        tbody.appendChild(tr);
    });
};
...
window.resetFormDepense = function() { 
    ...
    if(btn) btn.innerHTML = "SAUVER"; 
    const btnCancel = document.getElementById('btn-cancel-depense');
...
window.supprimerDepense = async (id) => { 
...
function updateFinancialDashboard() {
    ...
    const elDep = document.getElementById('stat-depenses');
    const elRes = document.getElementById('stat-resultat');
    
    if(elCA) elCA.innerText = global_CA.toFixed(2) + " €";
    if(elDep) elDep.innerText = global_Depenses.toFixed(2) + " €";
    
    if(elRes) {
        const res = global_CA - global_Depenses;
        elRes.innerText = res.toFixed(2) + " €";
        elRes.style.color = res >= 0 ? '#1e40af' : '#b91c1c';
    }
}
...
async function chargerSuggestionsClients() { 
    try { 
        ...
        const snap = await getDocs(q); 
        const dl = document.getElementById('clients_suggestions'); 
        if(dl) dl.innerHTML = ""; 
        snap.forEach(doc => { 
            if(doc.data().mandant?.nom) {
                const opt = document.createElement('option');
                opt.value = doc.data().mandant.nom;
...
// Fonction utilitaire pour le bouton PDF
