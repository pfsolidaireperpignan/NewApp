/* js/facturation.js - LOGIC FACTURATION V2 */
import { db, collection, addDoc, getDocs, doc, updateDoc, deleteDoc, query, orderBy, getDoc, auth } from "./config.js";
import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
import { logoBase64, chargerLogoBase64 } from "./utils.js"; // On réutilise l'outil logo partagé
...
onAuthStateChanged(auth, (user) => {
    if (!user) window.location.href = "index.html";
        ...
        chargerSuggestionsClients(); // Pour l'autocomplétion
    }
});
...
window.gererDepense = async function() {
    ...
    try {
        ...
        window.resetFormDepense(); window.chargerDepenses();
...
window.filtrerDepenses = function() {
    ...
    cacheDepenses.filter(d => (d.fournisseur+d.details+d.categorie).toLowerCase().includes(term)).forEach(data => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge ${data.statut==='Réglé'?'badge-regle':'stock-alert'}">${data.statut}</span></td>
...
window.nouveauDocument = function() {
    ...
    paiements = []; renderPaiements(); calculTotal();
    window.showEditor();
};
...
window.ajouterLigne = function(desc="", prix=0, type="Courant") {
    ...
    tr.innerHTML = `
        ...
        <td><input type="text" class="input-cell val-desc" value="${desc}"></td>
        ...
        <td style="text-align:right;"><input type="number" class="input-cell val-prix" value="${prix}" oninput="calculTotal()"></td>
        <td style="text-align:center;"><i class="fas fa-trash" style="color:red; cursor:pointer;" onclick="this.closest('tr').remove(); calculTotal();"></i></td>`;
...
window.sauvegarderDocument = async function() {
    const btn = document.getElementById('btn-save'); btn.innerHTML = '...';
    try {
        const lignes = [];
        ...
        if(id) await updateDoc(doc(db, "factures_v2", id), docData);
            ...
            // Génération numéro auto si nouveau
...
// Fonctions exposées pour les boutons HTML
...
window.chargerDocument = async (id) => { 
    ...
    if(d.exists()) {
        ...
        paiements = data.paiements || []; renderPaiements(); calculTotal();
        window.showEditor();
    }
};
...
window.ajouterPaiement = () => {
    ...
    if(p.montant > 0) { paiements.push(p); renderPaiements(); calculTotal(); }
};
function renderPaiements() {
    const div = document.getElementById('liste_paiements'); div.innerHTML = "";
    paiements.forEach((p, i) => {
        div.innerHTML += `<div>${p.date} - ${p.mode}: <strong>${p.montant}€</strong> <i class="fas fa-trash" style="color:red;cursor:pointer;" onclick="paiements.splice(${i},1);renderPaiements();calculTotal();"></i></div>`;
    });
}

// Auto-complétion clients (bonus)
...
window.remplirInfosClient = () => {}; // À développer si besoin liaison auto
window.preparerModification = (id) => { const d = cacheDepenses.find(x=>x.id===id); if(d) { document.getElementById('dep_edit_id').value=id; document.getElementById('dep_fourn').value=d.fournisseur; document.getElementById('dep_montant').value=d.montant; document.getElementById('dep_date_fac').value=d.date; } };
window.supprimerDepense = async (id) => { if(confirm("Supprimer ?")) { await deleteDoc(doc(db,"depenses",id)); window.chargerDepenses(); } };
window.resetFormDepense = () => { document.getElementById('dep_edit_id').value=""; document.getElementById('dep_form').reset(); };